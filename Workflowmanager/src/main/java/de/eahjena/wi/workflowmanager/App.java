...
public class App implements EventBusListener
{
	...
	public void messageReceived(String topic,String message)
	{
		...
		else if(topic.startsWith("workflow/claim/user"))
		{
			...
			String[] taskIds = null;
			try{
				taskIds = mapper.readValue(message, String[].class);
			}
			catch(Exception exc)
			{
				log.error(exc);
			}
			log.debug("found "+taskIds.length+" taskIds to be claimed...");
			//claim tasks
			for(String thisTaskId : taskIds)
			{
